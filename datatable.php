<?php 
	...
	$result = mysqli_query($con, 'SELECT count(ma) as total FROM quatrinhhoctap');
...
<body>
	<div class="container pt-3">
		<h3 class="text-center text-monospace pb-2">Quá trình học tập</h3>
		...
		<div class="shadow">
			<table class="table table-bordered mt-4">
				...
				<tbody>
					<?php 
						include_once 'conn.php';
						$q = " SELECT * FROM quatrinhhoctap LIMIT $start, $limit";
		...
		<div class=" pagination d-flex justify-content-center align-items-center pt-2">
			...
			if ($current_page > 1 && $total_page > 1){
				echo '<a href="datatable.php?page='.($current_page-1).'"><i class="fas fa-angle-double-left"></i> Previous</a>  ';
			}
			...
			for ($i = 1; $i <= $total_page; $i++){
				if ($i == $current_page){
					...
					echo '<a href="datatable.php?page='.$i.'">'.$i.'</a>  ';
				}
			}

			if ($current_page < $total_page && $total_page > 1){
				echo '<a href="datatable.php?page='.($current_page+1).'">Next <i class="fas fa-angle-double-right"></i></a>  ';
			}
			?>
